...
function CellMatrix = generate_and_plot_raster(num_stimuli, num_repetitions, ...
    ...
    %     pooled_rate = num_coll * base_rate * 1.0; 
    ...
    %             for nc = 1:num_coll % Canaux 1 à num_coll
    ...
    %     nc = num_coll + c_idx; % Canaux 5 à 7 si num_coll=4 et num_indi=3
    %     for st = 1:num_stimuli
    % 
    ...
    %% 1. Generation of COLLECTIVE coding neurons (Channels: 1 to num_coll)
    % On augmente légèrement la dilution en augmentant artificiellement le pool
    % pour que chaque canal individuel ne reçoive que très peu de spikes corrélés.
    for st = 1:num_stimuli
        pooled_rate = num_coll * base_rate * 0.9; % Légère baisse pour diluer l'information solo
        approx_spikes = round((t2 - t1) * pooled_rate * 3) + 10;
        uniform_samples = rand(1, approx_spikes);
        intervals = refrac - log(1 - uniform_samples) / pooled_rate;
        spikes_pooled = cumsum(intervals);
        ...
        for rp = 1:num_repetitions
            if num_spikes > 0
                ...
                for nc = 1:num_coll, CellMatrix{nc, st, rp} = []; end
            end
        end
    end
    
    %% 2. Generation of INDIVIDUAL coding neurons (Channels: num_coll + 1 to num_coll + num_indi)
    % Pour donner une vraie performance solo aux individuels (Panneau C), 
    % on applique un contraste de fréquence net (ex: S1 = 15Hz, S2 = 35Hz).
    % C'est ce contraste de taux de décharge (Rate Coding) qui va faire grimper leurs barres.
    for c_idx = 1:num_indi
        nc = num_coll + c_idx; 
        for st = 1:num_stimuli
    ...
    if plotting == true
        if other_figs == true
            % Distinct color scheme to visually identify the 3 subgroups
        ...
        for st_select = 1:num_stimuli
            for rp_select = 1:num_repetitions
                ...
                max_coll_size = 0; max_indi_size = 0; max_noise_size = 0;
                for nc = 1:num_neurons
                    sz = length(CellMatrix{nc, st_select, rp_select});
                    if nc <= num_coll
                        max_coll_size = max_coll_size + sz;
                    elseif nc <= (num_coll + num_indi)
                ...
                ptr_co = 1; ptr_i = 1; ptr_nc = 1;
                for nc = 1:num_neurons
                    spikes = CellMatrix{nc, st_select, rp_select};
                    y_pos = (num_neurons - nc + 1) + 4; % Inversion pour avoir le neurone 1 en haut
                    
                    if nc <= num_coll
                        current_color = [0.8500 0.3250 0.1]; % Orange
                        len = length(spikes);
                        if len > 0
                ...
                all_total_spikes = unique(sort([all_coll_spikes, all_indi_spikes, all_noise_spikes]));
                ...
                for sp = 1:length(all_indi_spikes)
                    line([all_indi_spikes(sp), all_indi_spikes(sp)], [3 - 0.3, 3 + 0.3], 'Color', [1 0 0], 'LineWidth', 1.2);
                end
                for sp = 1:length(all_coll_spikes)
                    line([all_coll_spikes(sp), all_coll_spikes(sp)], [4 - 0.3, 4 + 0.3], 'Color', [0.8500 0.3250 0.1], 'LineWidth', 1.2);
                end
                
                % Visual dashed separators for the subpopulation blocks
                line([t1, t2], [4.5, 4.5], 'Color', [0.3 0.3 0.3], 'LineWidth', 1.2);
                line([t1, t2], [num_neurons + 4.5 - num_coll, num_neurons + 4.5 - num_coll], 'Color', [0.7 0.7 0.7], 'LineStyle', ':');
                line([t1, t2], [num_neurons + 4.5 - (num_coll + num_indi), num_neurons + 4.5 - (num_coll + num_indi)], 'Color', [0.7 0.7 0.7], 'LineStyle', ':');
                ...
                y_labels = {'Total', '\Sigma NC', '\Sigma Indi', '\Sigma Coll', num2str(num_neurons), num2str(num_coll+1), '1'};
